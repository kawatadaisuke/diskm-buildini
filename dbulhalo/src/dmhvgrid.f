...
c *** get sigma phi
      open(60,file='vph.dat',status='unknown') 
...
c      close(60)
      ...
      if(myrank.eq.0) then
      open(60,file='dmhvgridz0.dat',status='unknown')
      open(61,file='dmhvgrid.dat',status='unknown')
      npg=(ngr+1)*(ngz+1)
c      j=0
      do j=0,ngz
        do i=0,ngr
     ...
     $         ,vsigph_rzg(pn),vsigz_rzg(pn)
          ...
          write(61,'(9(1pE13.5))') x_rzg(pn),z_rzg(pn)
     &         ,VUKMS*dsqrt(vc2_rzg(pn)),vph_rzg(pn),vsigr_rzg(pn)
